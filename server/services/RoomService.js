...
RoomService.messageRoomsWithUser = function(spec) {
	...
	RoomMember.find()
		.where(query)
		.populate("room")
		.exec(function(error, roomMembers) {
...
RoomService.messageUserInRoom = function(userId, roomId, messageText, type) {
	return RoomMember.findOne({ room: roomId, user: userId })
		...
		.then(function(roomMember) {
			socketio.io.to("userself_" + roomMember.user._id).emit("roommember", {
				_id: roomMember._id,
				verb: "messaged",
				data: {
					_id: uuid.v4(),
...
RoomService.setRoomAttribute = (roomMember, text) => {
	if (roomMember.role === "member")
		throw new ForbiddenError("Must be an administrator or moderator to change room attributes");
	...
	return Room.findById(roomMember.room)
		.then(room => {
			...
			if (command === "name") {
				...
				if (roomMember.role !== "administrator")
					throw new ForbiddenError("Must be an administrator to change room icon");
				...
				if (!icon || icon.length === 0) {
					...
					if (!icon.startsWith(":fa-"))
						throw new InvalidInputError("Invalid icon — use Font Awesome icons (they start with :fa-)");
