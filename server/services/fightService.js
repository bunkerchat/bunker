...
module.exports.play = function(roomMember, command) {
	...
	if (!parameters || parameters.length != 5) {
		throw new InvalidInputError(
			"Cannot start the fight — Bad input, see the help topic on fight (/help fight) for correct parameter usage"
		);
	}
	...
	return RoomMember.find({ room: roomMember.room })
		.populate("user")
		.then(roomMembers => {
			...
			if (roomMember.user._id.equals(opponentRoomMember.user._id)) {
			...
			return getFight(roomMember.user._id, opponentRoomMember.user._id, roomMember.room).then(function(fight) {
				if (!fight) {
					throw new InvalidInputError(
						"Cannot start the fight — unable to challenge " + opponentNick + ", there is already an active fight"
					);
				}
			...
			return User.find({ _id: { $in: userIds } }).then(users => {
				...
				_.forEach(myChallenges, function(challenge) {
					var userIndex = _.findIndex(users, function(user) {
						return user._id.equals(challenge.opponent);
					});

					slackers.push(users[userIndex].nick);
				});

				slackers = _.uniq(slackers);
				...
				_.forEach(mySlacking, function(unrespondedTo) {
					var userIndex = _.findIndex(users, function(user) {
						return user._id.equals(unrespondedTo.challenger);
					});

					myUnresponded.push(users[userIndex].nick);
				});

				myUnresponded = _.uniq(myUnresponded);
...
function buildChallengeResponse(fight) {
	return Promise.join(User.findOne({ _id: fight.challenger }), User.findOne({ _id: fight.opponent })).spread(function(
		challenger,
		opponent
	) {
...
function buildFightResultsResponse(fight, rounds) {
	return Promise.join(
		...
		if (fightResultData.winner) {
			if (fightResultData.winner._id.equals(challenger._id)) {
				responseString.push(
					challengerNick +
						" wins the fight " +
						fightResultData.challengerWins +
						" to " +
						fightResultData.opponentWins +
						".  :" +
						getFatalityEmote() +
						":"
				);
...
function getTieEmote() {
	return _.sample(["tie0", "tie1", "tie2"]);
}
