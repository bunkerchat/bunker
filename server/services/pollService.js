...
pollService.poll = (roomMember, text) => {
	...
	return pollService.start(roomMember, text).then(function(pollResponse) {
...
pollService.pollClose = (roomMember, text) => {
	const roomId = roomMember.room;
	return pollService.close(roomMember, text).then(function(pollResponse) {
		if (pollResponse.isPrivate) {
			RoomService.messageUserInRoom(roomMember.user._id, roomMember.room, pollResponse.message);
		} else {
			RoomService.messageRoom(roomId, pollResponse.message);
...
pollService.vote = (roomMember, text) => {
	const roomId = roomMember.room;
	return pollService.vote(roomMember, text).then(function(pollResponse) {
		RoomService.messageUserInRoom(roomMember.user._id, roomMember.room, pollResponse.message);
	});
};

pollService.start = function(roomMember, command) {
	var match = /^\/poll?(?:\s+(.+)?|$)/gi.exec(command);
...
pollService.vote = function(roomMember, command) {
	var match = /^\/vote?(?:\s+(.+)?|$)/gi.exec(command);
	...
	return Poll.findOne({ room: roomMember.room, isOpen: true })
		...
		.then(function(activePoll) {
			...
			if (respondees.indexOf(roomMember.user._id) != -1) {
				...
				return PollOption.findOne({ poll: activePoll._id, optionNumber: optionNumber }).then(function(selectedOption) {
					if (!selectedOption) {
						var isPrivate = true;
...
pollService.close = function(roomMember, command) {
	var match = /^\/poll(\s?)close?(?:\s*)/gi.exec(command);
	var optionNumber = match[1];
	...
	return Poll.findOne({ room: roomMember.room, isOpen: true })
		...
		.then(function(activePoll) {
			...
			if (roomMember.role == "administrator" || roomMember.role == "moderator" || ownerId.equals(userId)) {
				...
				var isPrivate = false;
				return Promise.join(activePoll.save(), finalPollResults(activePoll, isPrivate)).spread(function(
					poll,
					response
				) {
					return response;
...
function finalPollResults(newPoll, isPrivate) {
	...
	return PollOption.find({ poll: newPoll._id }).then(function(pollOptions) {
