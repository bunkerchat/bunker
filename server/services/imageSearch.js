...
imageSearch.image = (roomMember, text) => {
	...
	return image(searchQuery).then(result => {
...
imageSearch.gif = (roomMember, text) => {
	...
	return gif(searchQuery).then(result => {
		socketio.io.to("userself_" + roomMember.user._id).emit("user", {
			_id: roomMember.user._id,
			verb: "messaged",
			data: {
...
function image(query) {
	return googleImageSearch(query).catch(() => ({ provider: "none", images: [] }));
}

function gif(query) {
	return googleImageSearch(query, true).catch(() => ({ provider: "none", images: [] }));
}

function googleImageSearch(query, animated) {
	const key = `imageSearch/google-${animated ? "gif" : "image"}|${query}`;
	return ImageCache.findOne({ key: key }).then(dbCache => {
		if (dbCache) {
