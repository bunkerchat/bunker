...
module.exports.message = function(req, res) {
	...
	return RoomMember.find({ room: roomId })
		.populate("user")
		.then(roomMembers => {
		...
		.spread((userUpdate, message, roomMembers) => {
			...
			_(roomMembers)
				.filter(roomMember => {
					...
					return (
						!user.connected || !user.present || !user.activeRoom || user.activeRoom.toString() !== roomId.toString()
					);
				})
				.each(roomMember => {
					...
					roomMember.unreadMention =
						unreadMessageCount === 1 && !mentioned ? false : roomMember.unreadMention || mentioned;

					return roomMember.save().then(() => {
...
module.exports.join = function(req, res) {
	...
	Promise.join(Room.findById(roomId), RoomMember.count({ room: roomId, user: userId }))
	...
	RoomMember.count({ room: roomId, user: userId })
		.then(function(existingRoomMember) {
			...
			return RoomMember.remove({ room: roomId, user: userId })
				.then(function() {
					return [User.findById(userId), RoomMember.find({ room: roomId }).populate("user")];
				})
...
module.exports.pinMessage = function(req, res) {
	...
	RoomMember.findOne({ room: roomId, user: userId })
		...
		.then(function(roomMember) {
			...
			return [
				PinnedMessage.create({ message: messageId, room: roomId, user: userId }),
